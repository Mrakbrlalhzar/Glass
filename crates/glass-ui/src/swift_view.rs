//! Tab renderer for `TabKind::SwiftType`.
//!
//! Parallels [`crate::objc_view`] for the Swift side. Operates on
//! pre-rendered [`SwiftRow`](glass_arch_arm::swift_format::SwiftRow)
//! values cached on the bundle in `LoadedBundle::swift_types`. The
//! address chunks emitted by `swift_format` (metadata-accessor +
//! vtable IMPs) use `ChunkKind::Address` with `target =
//! Some(vaddr)`; clicking them resolves the artifact's text section
//! for that address and opens / focuses a Listing tab scrolled to
//! the IMP. Shift-click forces a new tab — same UX as listing
//! operand clicks.
...
pub(crate) fn render_swift_tab(
    ...
    let (artifact, mangled_name) = match shell
        .active_tab
        .and_then(|i| shell.tabs.get(i))
        .map(|t| t.kind.clone())
    {
        Some(TabKind::SwiftType { artifact, mangled_name }) => (artifact, mangled_name),
        _ => return div().flex_1().into_any_element(),
    };

    let rows: Arc<Vec<glass_arch_arm::swift_format::SwiftRow>> = bundle
        .swift_types
        .get(&(artifact.clone(), mangled_name.clone()))
        .cloned()
...
fn render_row(
    row: &glass_arch_arm::swift_format::SwiftRow,
    index: usize,
